...
# import the output blast tabular files of reads against sequences (genomic contigs) from 3 species (e.g. virus, host species, and a species whose DNA/RNA has been added as a control for artificial chimeras)

#blastResults = lapply(c("blastOnconfident_TE.cons.fa_ALL.txt", "blastOnSeMNPV_ALL.txt"), function(path) fread(path = header = F, sep = "\t"))
#species = c("SexiguaTEs", "SeMNPV")			#the target species corresponding to the blast results above
#nrows = sapply(blastResults, nrow)					#numbers of HSPs (rows) in each blast result



blast1=fread("/home/olivier/repositories/nf-chimera-detector/tests/test_data/blast_outputs/chimeras/blastOnTEs.txt")

blast2=fread("/home/olivier/repositories/nf-chimera-detector/tests/test_data/blast_outputs/chimeras/blastOnVirusGenome.txt")

blast1$Sp="TEs"
setnames(blast1, c("readName", "subject", "identity", "alength", "mismatch", "indel", "qStart", "qEnd", "sStart", "sEnd", "score", "readLength", "Sp"))

blast2$Sp="Virus"
setnames(blast2, c("readName", "subject", "identity", "alength", "mismatch", "indel", "qStart", "qEnd", "sStart", "sEnd", "score", "readLength", "Sp"))
...
write.table(chim_mCATblastn_noOverlap, "chimeras.csv", sep = ',', row.names = FALSE, quote = FALSE)
